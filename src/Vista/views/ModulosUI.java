...
import Controlador.controllers.AlumnosController;
...
public abstract class ModulosUI {
	 ...
	 */
	protected abstract void agregarModulo(String nombre, int curso, int horas, int idCiclo);
	/**
	 * Edita un modulo seleccionado en funcion de los valores de los campos:
	 * nombre, ciclo, curso y horas, id
	 */
	protected abstract void editarModulo(Integer idModulo, String nombre, int curso, int horas, int idCiclo);
	...
	private void initialize() {
		...
		btnGuardar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if (id == null) {
					...
					try {
						agregarModulo(nombreTextField.getText(), Integer.parseInt(cursoTextField.getText()), horas,
								cicloComboBox.getSelectedIndex() + 1);
					} catch (NumberFormatException e) {
						System.out.println("Error, el valor introducido no es un digito.");
					}
					...
					try {
						editarModulo(id, nombreTextField.getText(), Integer.parseInt(cursoTextField.getText()), horas,
								cicloComboBox.getSelectedIndex() + 1);
					} catch (NumberFormatException e) {
						System.out.println("Error, el valor introducido no es un digito.");
					}
		...
		btnGuardar.setBounds(250, 93, 89, 23);
		...
		btnLimpiar.setBounds(351, 93, 89, 23);
		frame.getContentPane().add(btnLimpiar);

		btnEliminar = new JButton("Eliminar");
		btnEliminar.setBounds(450, 93, 89, 23);
	...
	private void recargarTabla() {
		...
		tm.addColumn("Horas");
		tm.addColumn("ID");
		for (ModuloVO modulo : transformarListaVO()) {
			tm.addRow(new String[] { modulo.getNombre(), getCicloNombre(modulo.getIdCiclo()),
					String.valueOf(modulo.getCurso()), String.valueOf(modulo.getHoras()),
					String.valueOf(modulo.getId()) });
		}
	...
	private void alumnosUI() {
		// TODO Lanzar interfaz alumnos

		AlumnosController aluController=new AlumnosController(id);
		aluController.show();
	}
